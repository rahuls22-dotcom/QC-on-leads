...
const inputCls =
  "w-full h-9 px-3 text-[13px] bg-white border border-border rounded-button text-text-primary focus:outline-none focus:border-accent placeholder:text-text-tertiary";
const taCls =
  "w-full px-3 py-2.5 text-[13px] bg-white border border-border rounded-input text-text-primary focus:outline-none focus:border-accent placeholder:text-text-tertiary resize-y leading-relaxed";
...
function SettingsBlock({
  ...
  return (
    <div className="space-y-3">
      <h3 className="text-[12px] font-semibold uppercase tracking-[0.04em] text-text-secondary">
        {title}
      </h3>
      {children}
    </div>
...
function TransferSettings({ settings, patch }: SettingsProps) {
  ...
  return (
    <SettingsBlock title="Transfer destinations">
      ...
      {destinations.map((d) => (
        <div key={d.id} className="border border-border rounded-card p-3.5 space-y-3">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-2 text-text-tertiary hover:text-[#DC2626] hover:bg-[#FEF2F2] rounded-button transition-colors shrink-0"
            >
      ...
      <button
        type="button"
        onClick={add}
        className="inline-flex items-center gap-1.5 text-[13px] font-medium text-accent hover:opacity-80 transition-opacity"
      >
        ...
        {LANGUAGE_OPTIONS.map((lang) => {
          ...
          return (
            <button
              ...
              type="button"
              onClick={() => toggle(lang)}
              className={`text-[12px] font-medium px-2.5 py-1 rounded-badge border transition-colors ${
                on
...
function CentresSettings({ settings, patch }: SettingsProps) {
  ...
  return (
    <SettingsBlock title="Experience centres">
      {centres.map((c) => (
        <div key={c.id} className="border border-border rounded-card p-3.5 space-y-2">
          <div className="flex items-center gap-2">
            <input
              value={c.name}
...
function Radio({
  ...
  return (
    <button
      type="button"
      onClick={onSelect}
      className={`w-full text-left flex gap-2.5 items-start border rounded-card px-3.5 py-3 transition-colors ${
        checked ? "border-accent bg-accent/[0.03]" : "border-border hover:border-border-hover"
      }`}
    >
      <span
      ...
      <span>
        ...
        <span className="block text-[11px] text-text-tertiary mt-0.5">{hint}</span>
      </span>
    </button>
  );
}
